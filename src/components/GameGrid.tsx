...
export const GameGrid: React.FC<GameGridProps> = ({
  ...
  const dotOffset = '0.625rem'; // 10px in rem units for better responsiveness
        ...
        Array.from({ length: size - 1 }, (_, col) => {
          ...
          return (
            <motion.button
              ...
              style={{
                left: `calc(${col * spacing}px + ${dotSize}px + ${dotOffset})`,
        ...
        Array.from({ length: size }, (_, col) => {
          ...
          return (
            <motion.button
              ...
              className={`absolute ${getLineColor(line)} rounded-full transition-colors duration-150`}
              style={{
                left: `calc(${col * spacing}px + (${dotSize - lineThickness}px)/2 + ${dotOffset})`,
                top: `calc(${row * spacing}px + ${dotSize}px + ${dotOffset})`,
                width: lineThickness,
      ...
      {Array.from({ length: size }, (_, row) =>
        Array.from({ length: size }, (_, col) => (
          <div
            ...
            style={{
              left: `calc(${col * spacing}px + ${dotOffset})`,
              top: `calc(${row * spacing}px + ${dotOffset})`,
              width: dotSize,
              height: dotSize,
